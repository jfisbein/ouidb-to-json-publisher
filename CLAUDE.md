...
**Where it runs instead:** the nightly job runs on the maintainer's Oracle Cloud host `ampere-03` (a "clean" IP the WAF does not block), under `~/ouidb-publisher/` (docker-compose using `:edge`, scheduled via cron at 06:00 UTC). The host is ARM64 and the image is multi-arch, so it runs natively (no emulation).

**Docker image:** built and pushed by `.github/workflows/maven.yml` on push to `master` (tag `:edge`) and on git tags (versioned tags + `:latest`). It is **multi-arch** (`linux/amd64,linux/arm64`). The Maven build stage is pinned to `$BUILDPLATFORM` so it builds once natively on the CI runner; only the final JRE stage (a portable-jar `COPY`, no `RUN`) is produced per target arch, so multi-arch adds negligible build cost.
